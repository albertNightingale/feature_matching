...
def point_line_distance(line, point):
    ...
    error = np.dot(line, point) / math.sqrt(a ** 2 + b ** 2)
...
def signed_point_line_errors(x_0s, F, x_1s):
    ...
    #######################################################################    
    n = np.array(x_0s).shape[0]
    errors = np.zeros((n, 2))
    
    for i in range(n):
        x_1 = x_1s[i]
        x_0 = x_0s[i]
        # getting l_0 using the F and compute the distance between l_0 and x_0
        l_0 = np.dot(F, x_1)
        # compute the distance between l_0 and x_0
        errors[i][0] = point_line_distance(l_0, x_0)
        
        # getting l_1 using the F and compute the distance between l_1 and x_1
        l_1 = np.dot(np.transpose(F), x_0)
        # compute the distance between l_1 and x_1
        errors[i][1] = point_line_distance(l_1, x_1)      
